...
export default function FineList() {
    ...
    const filteredFines = fines.filter((record) => {
        ...
        const matchesSearch = record.patron.name.toLowerCase().includes(term) || record.patron.school_id.toLowerCase().includes(term);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center">
                <input
                    ...
                    className="w-120 rounded border border-black px-2 py-2"
                />

                <Select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value as 'all' | 'unpaid' | 'cleared')}
                    className="rounded border border-black px-2 py-2"
                >
                ...
                <button
                    ...
                    className={`rounded px-4 py-2 text-white ${
                        filteredFines.length ? 'bg-green-600 hover:bg-green-700' : 'cursor-not-allowed bg-gray-300'
                    }`}
                >
                    Export to Excel
                </button>
            ...
            {/* Table */}
            <div className="mb-6 overflow-x-auto rounded bg-white p-6 shadow-lg">
                <table className="w-full border-collapse rounded-lg bg-white text-black shadow-sm">
                    <thead>
                        <tr className="border-b bg-purple-900 text-white">
                            {['Borrower Info', 'Book', 'Due Date & Time', 'Returned Date', 'Fine Amount', 'Fine Status', 'Action'].map((header) => (
                        ...
                        {displayedFines.length > 0 ? (
                            displayedFines.map((record) => (
                                <tr key={record.id} className="border-b hover:bg-gray-200">
                                    ...
                                    <td className="p-3">
                                        <span className={`rounded px-2 py-1 text-sm text-white ${getFineStatusColor(record.fine_status)}`}>
                                            {record.fine_status}
                                        </span>
                                    </td>
                                    <td className="p-3">
                                        <button
                                            ...
                                            className="mr-2 mb-2 rounded bg-blue-600 px-3 py-1 text-sm text-white hover:bg-blue-700"
                                        >
                                            Print Receipt
                                        </button>
            ...
            <FineListModal show={showModal} patronName={selectedFine?.patron.name} onClose={() => setShowModal(false)} onConfirm={handleConfirm} />
